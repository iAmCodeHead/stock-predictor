...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
           const url = `${CLOUDFLARE_POLYGON_API_WORKER_URL}/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
...
async function fetchReport(data) {
    ...
    try {
        const url = process.env.CLOUDFLARE_OPEN_API_URL
